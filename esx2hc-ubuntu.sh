...
#
# Ubuntu-specific version with nbdkit VDDK plugin support
#
...
usage() {
  cat <<'EOF'
Usage: esx2hc-ubuntu.sh [options]
...
Ubuntu 24.04 Notes:
  - This script automatically handles LD_LIBRARY_PATH for VDDK
  - Runs virt-v2v with sudo for kernel access (required by libguestfs)
  - Requires nbdkit-vddk-plugin to be built (see README-Ubuntu.md)
EOF
}
...
check_prerequisites() {
  log "Checking prerequisites (Ubuntu)"
  local mgr
  mgr="$(detect_pkg_mgr)"
  
  if [[ "$mgr" != "apt" ]]; then
    err "This script is for Ubuntu/Debian systems only"
    err "For RHEL/AlmaLinux/Rocky, use esx2hc.sh instead"
    exit 1
  fi
  ...
  # Check for nbdkit and nbdkit-vddk-plugin
  if need nbdkit; then
    printf "  %-18s %s\n" "nbdkit" "OK"
  else
    printf "  %-18s %s\n" "nbdkit" "MISSING"
  ...
  if [[ "${#missing[@]}" -gt 0 ]]; then
    ...
    install_packages apt curl jq python3 virt-v2v libguestfs-tools libvirt-clients \
  ...
  if [[ -n "$VDDK_LIBDIR" ]]; then
    ...
    if [[ -f "/usr/lib/x86_64-linux-gnu/nbdkit/plugins/nbdkit-vddk-plugin.so" ]]; then
      ...
      err "nbdkit-vddk-plugin not found at /usr/lib/x86_64-linux-gnu/nbdkit/plugins/"
      err ""
      err "Ubuntu 24.04 requires building nbdkit-vddk-plugin from source."
      err "Follow the instructions in README-Ubuntu.md Step 6 to build it."
      err ""
      err "Quick build steps:"
      err "  sudo sed -i 's/^Types: deb\$/Types: deb deb-src/' /etc/apt/sources.list.d/ubuntu.sources"
      err "  sudo apt update"
      err "  sudo apt install -y nbdkit-plugin-dev build-essential autoconf automake libtool pkg-config"
      err "  cd /tmp && apt source nbdkit && cd nbdkit-*"
      err "  ./configure --with-vddk=$VDDK_LIBDIR"
      err "  make"
      err "  sudo cp plugins/vddk/.libs/nbdkit-vddk-plugin.so /usr/lib/x86_64-linux-gnu/nbdkit/plugins/"
      exit 1
    fi
  fi

  # Check kernel readability
  if [[ "$DRY_RUN" -eq 0 ]]; then
    log "Checking kernel file permissions..."
    if ls -la /boot/vmlinuz-* 2>/dev/null | grep -q "^-r--------"; then
      warn "Kernel files are not readable (will cause libguestfs to fail)"
      err ""
      err "Fix with: sudo chmod +r /boot/vmlinuz-*"
      err ""
      err "This is required for libguestfs/supermin to build the appliance."
      exit 1
    else
      printf "  %-18s %s\n" "kernel readable" "OK"
    fi
...
if [[ "$DRY_RUN" -eq 1 ]]; then
  ...
  if [[ -n "$VDDK_LIBDIR" ]]; then
    ...
    # Use base VDDK directory (nbdkit will append /lib64 automatically)
    CMD+=(-it vddk -io "vddk-libdir=$VDDK_LIBDIR" -io "vddk-thumbprint=$VDDK_THUMBPRINT")
  else
...
log "      2. Boot Maintenance Manager → Boot From File"
